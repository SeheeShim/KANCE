...
const Challenge = () => {
  ...
  const fetchVideos = async (type) => {
    ...
    if (type === "Trending") {
      // 트렌딩 비디오
      url = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=kpopdance&maxResults=12&order=viewCount&type=video&videoCategoryId=10&regionCode=KR&key=${API_KEY}`;
    } else if (type === "New") {
      // 신곡 비디오
      url = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=kpopdance&maxResults=12&order=date&type=video&videoCategoryId=10&regionCode=KR&key=${API_KEY}`;
    }
  ...
  const handleVideoClick = (videoId) => {
    ...
    // 비디오 클릭 시 유튜브 쇼츠 검색 결과로 이동
  const handleVideoClick = (videoId, title) => {
    // 곡명(title)로 유튜브 쇼츠 검색
    const query = encodeURIComponent(`${title} shorts`);
    ...
    <div>
        ...
        <div className="trending-grid">
          {getFilteredVideos().map((video, idx) => (
            <div
              ...
              key={video.id.videoId || video.id}
              className="challenge-card"
              onClick={() => handleVideoClick(video.id.videoId, video.snippet.title)}
              style={{ cursor: "pointer" }}
            >
              <img
      ...
      <div className="scoring-section">
        ...
        <div className="video-container">
          ...
          >
            <source 
              src={`${process.env.PUBLIC_URL}/video/Kance_AdVideo.mp4`} 
              type="video/mp4" 
            />
